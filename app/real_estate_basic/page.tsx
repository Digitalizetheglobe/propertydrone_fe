...
export default function RealEstateBasicsPage() {
  const [realEstateBasics, setRealEstateBasics] = useState<RealEstateBasic[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setLoading(true);
    fetch('api.propertydronerealty.com/real-estate', { cache: 'no-store' })
      .then(res => {
        if (!res.ok) throw new Error('Failed to fetch real estate basics');
        return res.json();
      })
      .then(data => {
        setRealEstateBasics(data);
        setLoading(false);
      })
      .catch(err => {
