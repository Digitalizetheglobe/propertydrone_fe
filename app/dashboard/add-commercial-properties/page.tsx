...
function AddCommercialPropertyContent() {
    ...
    useEffect(() => {
        if (editId) {
            const fetchDetails = async () => {
                try {
                    const response = await axios.get(`http://localhost:9000/api/commercial-properties/${editId}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editId) {
                await axios.put(`http://localhost:9000/api/commercial-properties/${editId}`, submitData, {
                    headers: { "Content-Type": "multipart/form-data" }
                });
                setMessage("Commercial Property updated successfully!");
            } else {
                await axios.post("http://localhost:9000/api/commercial-properties", submitData, {
                    headers: { "Content-Type": "multipart/form-data" }
                });
