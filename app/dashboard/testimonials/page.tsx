...
export default function TestimonialsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId !== null) {
        // Update
        res = await fetch(`http://localhost:9000/api/testimonials/${editId}`, {
          method: "PUT",
        ...
        // Create
        res = await fetch("http://localhost:9000/api/testimonials", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData)
        });
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      const res = await fetch(`http://localhost:9000/api/testimonials/${id}`, {
        method: "DELETE"
      });
