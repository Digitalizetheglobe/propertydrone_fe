...
export default function PropertyForm() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    amenities: [],
    configurationTypology: [
      { flat_available: '', area_sqft: '', price: '', note: '' }
    ],
    event: '',
    reraNumber: '',
    pros: [],
  ...
  const handleConfigChange = (idx: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData(prev => {
      const updated = [...prev.configurationTypology];
      updated[idx] = { ...updated[idx], [name]: value };
      return { ...prev, configurationTypology: updated };
    });
  };
  const addConfigRow = () => {
    setFormData(prev => ({
  ...
  />
        ...
        <div className="border-t pt-6 mt-6">
          ...
          {formData.configurationTypology.map((config, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-2">
              ...
              <div className="flex gap-2">
                ...
                />
                {formData.configurationTypology.length > 1 && (
                  <button type="button" onClick={() => removeConfigRow(idx)} className="px-2 py-1 bg-red-500 text-white rounded-md">-</button>
                )}
              </div>
            </div>
          ))}
          <button type="button" onClick={addConfigRow} className="mt-2 px-4 py-2 bg-blue-500 text-white rounded-md">Add Configuration</button>
        </div>
        {/* SEO Fields */}
        <div className="border-t pt-6 mt-6">
