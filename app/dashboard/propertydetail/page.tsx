...
export default function PropertyDetail() {
  ...
  const addToComparison = async (property: any) => {
    await fetch('api.propertydronerealty.com/api/property-comparisons', {
      method: 'POST',
  ...
  const fetchProperties = () => {
    setLoading(true);
    fetch("api.propertydronerealty.com/properties")
      .then((res) => res.json())
      .then((data) => {
  ...
  const deleteProperty = async (id: string) => {
    try {
      const response = await fetch(`api.propertydronerealty.com/properties/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleUpdateProperty = async () => {
    ...
    try {
      ...
      // Send request
      const response = await fetch(`api.propertydronerealty.com/properties/${editProperty.id}`, {
        method: "PUT",
        body: formData,
      });
