...
export default function ApplicationsDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch applications
        const appsResponse = await fetch('api.propertydronerealty.com/applications');
        const appsData = await appsResponse.json();
        setApplications(appsData);

        // Fetch available jobs for filter
        const jobsResponse = await fetch('api.propertydronerealty.com/careers');
  ...
  const handleDelete = async (id: string): Promise<void> => {
    if (confirm('Are you sure you want to delete this application?')) {
      try {
        const response: Response = await fetch(`api.propertydronerealty.com/applications/${id}`, {
          method: 'DELETE'
        });
  ...
  const updateStatus = async (id: string, newStatus: string): Promise<void> => {
    try {
      const response: Response = await fetch(`api.propertydronerealty.com/applications/${id}/status`, {
        method: 'PATCH',
        headers: {
