...
export default function ApplicationsDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch applications
        const appsResponse = await fetch('http://localhost:9000/applications');
        const appsData = await appsResponse.json();
        setApplications(appsData);

        // Fetch available jobs for filter
        const jobsResponse = await fetch('http://localhost:9000/careers');
  ...
  const handleDelete = async (id: string): Promise<void> => {
    if (confirm('Are you sure you want to delete this application?')) {
      try {
        const response: Response = await fetch(`http://localhost:9000/applications/${id}`, {
          method: 'DELETE'
        });
  ...
  const updateStatus = async (id: string, newStatus: string): Promise<void> => {
    try {
      const response: Response = await fetch(`http://localhost:9000/applications/${id}/status`, {
        method: 'PATCH',
        headers: {
