...
async function getProperty(slug: string) {
    ...
    if (isNumeric) {
        try {
            const res = await fetch(`http://localhost:5000/api/commercial-properties/${slug}`, { cache: 'no-store' });
    ...
    // Try by slug
    try {
        const res = await fetch(`http://localhost:5000/api/commercial-properties/slug/${slug}`, { cache: 'no-store' });
        if (res.ok) return res.json();
    } catch (e) { }
