                            ...
                            <input
                                ...
                                onChange={async (e) => {
                                    ...
                                    if (e.target.checked) {
                                        ...
                                        if (comparisonId) {
                                            await fetch(`http://api.propertydronerealty.com/api/property-comparisons/${comparisonId}`, { method: "DELETE" });
...
export default function SavedPropertiesPage() {
    ...
    const fetchSavedIds = async () => {
        if (!userId) return;
        try {
            const res = await fetch(`http://api.propertydronerealty.com/api/saved-properties?webUserId=${userId}`);
            if (res.ok) {
    ...
    const fetchComparedIds = async () => {
        if (!userId) return;
        try {
            const res = await fetch(`http://api.propertydronerealty.com/api/property-comparisons?webUserId=${userId}`);
            if (res.ok) {
                const data = await res.json();
